...
public class AdvancedWebView extends WebView {
    ...
    /**
     * {@link View#SYSTEM_UI_FLAG_FULLSCREEN}
     */
    public static final int SYSTEM_UI_FLAG_FULLSCREEN = 0x00000004;

    /**
     * {@link View#SYSTEM_UI_FLAG_IMMERSIVE}
     */
    ...
    protected static final int REQUEST_CODE_CAMERA = 51428;
    ...
    /**
     * Alternative browsers that have their own rendering engine and *may* be installed on this
     * device
     */
    protected static final String[] ALTERNATIVE_BROWSERS = new String[]{"org.mozilla.firefox",
    ...
    private void cameraSuccss(Intent intent) {
        Uri[] dataUris = null;
        if (mUri != null) {
            dataUris = new Uri[]{mUri};
        }
        if (mCustomListener != null) {
            mCustomListener.actionCallBack(dataUris);
        }
        if (intent != null) {
            ...
            if (mFileUploadCallbackFirst != null) {
                mFileUploadCallbackFirst.onReceiveValue(mUri);
                mFileUploadCallbackFirst = null;
            } else if (mFileUploadCallbackSecond != null) {
                mFileUploadCallbackSecond.onReceiveValue(dataUris);
                mFileUploadCallbackSecond = null;
     ...
     * images)
     *
     * If you later want to delete an HTTP header that was previously added this way, call
     * `removeHttpHeader()`
     *
     ...
     *
     * If you want to unset a pre-defined header, set it to an empty string with `addHttpHeader()`
     * instead
     *
     * The `WebView` implementation may in some cases overwrite headers that you set or unset
     *
    ...
    public void setDesktopMode(final boolean enabled) {
        ...
        if (enabled) {
            newUserAgent = webSettings.getUserAgentString().replace("Mobile", "eliboM")
                    .replace("Android", "diordnA");
        } else {
            newUserAgent = webSettings.getUserAgentString().replace("eliboM", "Mobile")
    ...
    protected void init(Context context) {
        ...
        final String databaseDir = filesDir.substring(0, filesDir.lastIndexOf("/"))
        ...
        super.setWebViewClient(new InnerWebViewClient());
        super.setWebChromeClient(new InnerWebChromeClient());
        ...
        setDownloadListener(new DownloadListener() {
            ...
            public void onDownloadStart(String url, String userAgent, String contentDisposition,
                    String mimetype, long contentLength) {
                if (mListener != null) {
                    mListener.onDownloadRequested(url, userAgent, contentDisposition, mimetype,
                            contentLength);
                }
            }
    ...
    protected void openFileInput(final ValueCallback<Uri> fileUploadCallbackFirst,
        ...
        if (mCustomListener != null) {
            //显示dialog 在dialog中调用 webview暴露出来的一些
            mCustomListener.actionOne();
        } else {
            defaultOpenFileAction();
        }


    }

    public void defaultOpenFileAction() {
        ...
        if (mFragment != null && mFragment.get() != null && Build.VERSION.SDK_INT >= 11) {
            mFragment.get()
                    .startActivityForResult(Intent.createChooser(i, getFileUploadPromptLabel()),
    ...
    private File createImageFile() throws IOException {
        File storageDir = new File(
                FileUtil.getBaseAndSysPath(mActivity.get(), "pictureCache/").getPath());
        long l = System.currentTimeMillis();
        String fileName = l + ".jpg";
        File mCurrentImageFile = new File(storageDir, fileName);
    ...
    interface CustomListener {
        ...
        void actionCallBack(Uri[] uris);
    }


    /**
     * Returns whether file uploads can be used on the current device (generally all platform
     * versions except for 4.4)
     *
     ...
     *
     * @param needsCorrectMimeType whether a correct MIME type is required for file uploads or
     *                             `application/octet-stream` is acceptable
     * @return whether file uploads can be used
     */
    ...
    /**
     * Handles a download by loading the file from `fromUrl` and saving it to `toFilename` on the
     * external storage
     *
     * This requires the two permissions `android.permission.INTERNET` and
     ...
     *                   `AdvancedWebView.onDownloadRequested(...)`
     * @param toFilename the name of the destination file where the download should be saved, e.g.
     *                   `myImage.jpg`
    ...
    public static class Browsers {
        ...
        /**
         * Returns whether there is an alternative browser with its own rendering engine currently
         * installed
         *
         * @param context a valid `Context` reference
        ...
        /**
         * Returns the package name of an alternative browser with its own rendering engine or
         * `null`
         *
         * @param context a valid `Context` reference
         ...
         * @param context           a valid `Activity` reference
         * @param url               the URL to open
         * @param withoutTransition whether to switch to the browser `Activity` without a
         *                          transition
         */
        public static void openUrl(final Activity context, final String url,
    ...
    public class InnerWebChromeClient extends WebChromeClient {
        ...
        public void onProgressChanged(WebView view, int newProgress) {
            if (mActivity != null && mActivity.get() != null) {
                mActivity.get().setProgress(newProgress * 100);
            }
        }

        @Override
        public void onReceivedTitle(WebView view, String title) {
            if (mActivity != null && mActivity.get() != null && !TextUtils.isEmpty(title)) {
        ...
        public void onShowCustomView(View view, CustomViewCallback callback) {
            ...
            mCustomView = view;
            mOriginalSystemUiVisibility = activity.getWindow().getDecorView()
    ...
    public class InnerWebViewClient extends WebViewClient {
        ...
        private static final String ERROR_URL_WITH_TITLE
                = "file:///android_asset/error_with_titleBar.html";

        private static final int DOUBLE_CLICK_TIME = 300;

        private String mUrl;

        private long mBackClickRecordTime;
        ...
        private static final String TAG = "InnerWebViewClient";
        ...
        public void onReceivedError(WebView view, int errorCode, String description,
            ...
            if (((AppCompatActivity) mActivity.get()).getSupportActionBar() != null) {
                view.loadUrl(ERROR_URL);
            } else {
                view.loadUrl(ERROR_URL_WITH_TITLE);
            }
        }
        ...
        public void onUnhandledInputEvent(WebView view, InputEvent event) {
            if (Build.VERSION.SDK_INT >= 21) {
                if (mCustomWebViewClient != null) {
                    mCustomWebViewClient.onUnhandledKeyEvent(view, (KeyEvent) event);

                } else {
                    super.onUnhandledKeyEvent(view, (KeyEvent) event);
                }
            }
        }
